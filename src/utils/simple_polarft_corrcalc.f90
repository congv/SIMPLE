...
type :: polarft_corrcalc
    ...
    complex(dp),         allocatable :: cavgs_num(:,:,:)            !< -"-, reference reg terms
...
contains
    ...
    subroutine accumulate_cavgs( self, eulspace, ptcl_eulspace, glob_pinds )
        ...
        complex(sp) :: ptcl_ctf_rot(self%pftsz,self%kfromto(1):self%kfromto(2))
        !$omp parallel do default(shared) private(i,iptcl,ithr,o_prev,loc,iref,ptcl_ctf_rot,ctf_rot) proc_bind(close) schedule(static)
        do i = 1, self%nptcls
            iptcl = glob_pinds(i)
            ithr  = omp_get_thread_num() + 1
            ! previous parameters
            ...
            call self%rotate_ptcl(self%pfts_ptcls(:,:,i) * self%ctfmats(:,:,i), loc, ptcl_ctf_rot)
    ...
    subroutine regularize_refs( self )
        ...
        do iref = 1, self%nrefs
            self%pfts_refs_even(:,:,iref) = self%pfts_refs_even(:,:,iref) - self%cavgs_num(:,:,iref)
            self%pfts_refs_odd( :,:,iref) = self%pfts_refs_odd( :,:,iref) - self%cavgs_num(:,:,iref)
        enddo
        !$omp end parallel do
